...
class PartnerXlsx(models.AbstractModel):
    _name = 'report.invoice_excel_report.invoice_xlsx'
        ...
        for obj in partners:
            ...
            if obj.state not in ['draft', 'sent']:
                worksheet.merge_range(
                    'A5:F5', 'Invoice :- ' + obj.name, order_format)
                worksheet.merge_range(
                    'C7:D7', 'Invoice Date', customer_header_format)
                worksheet.merge_range(
                    'E7:F7', str(obj.create_date.date()), customer_format)
            ...
            display_discount = any([l.discount for l in obj.invoice_line_ids])
            display_tax = any([l.tax_ids for l in obj.invoice_line_ids])
            ...
            worksheet.write(row, 2, 'Currency', table_header_right)
            ...
            for line in obj.invoice_line_ids:
                ...
                worksheet.write(row, 1, line.quantity, table_row_right)
                worksheet.write(row, 2, line.price_unit, table_row_right)
                worksheet.write(row, 2, line.currency_id.name, table_row_right)
                if display_discount and group:
                    worksheet.write(row, 3, line.discount, table_row_right)
                    if display_tax and line.tax_ids:
                        worksheet.write(
                            row, 4, line.tax_ids.name, table_row_right)
                        worksheet.write(
                            row, 5, line.price_subtotal, table_row_right)
                        row += 1
                    elif not line.tax_ids and display_tax:
                    ...
                    if display_tax and line.tax_ids:
                        worksheet.write(
                            row, 3, line.tax_ids.name, table_row_right)
                        worksheet.write(
                            row, 4, line.price_subtotal, table_row_right)
                        row += 1
